...
- `install.sh` — one-shot Claude-hook registration (edits
  `~/.claude/settings.json` since TPM can't), plus optional emoji
  picker fetch and theme recommendation.
- `mark.sh` / `mark_emoji.sh` — popup UIs for setting the per-pane
  `@agent-mark` option. Bound to `prefix+m` / `prefix+M` by default
  via `agent_tracker.tmux`.
- `emoji.txt` — bundled curated emoji list (~60 dev-relevant entries).
- `emoji_full.txt` — generated by `install.sh --emoji` from Unicode's
  `emoji-test.txt`. Not checked in.
...
### Marks bypass the provider pipeline deliberately

Per-pane marks (`@agent-mark` option) are queried synchronously in
`window_badge.sh` at render time rather than flowing through a
provider. Rationale: marks change when the user explicitly sets one,
not via any observation cycle — there's nothing for a provider to
observe. Running them through the refresh pipeline would add a
polling delay (up to `@window-badge-poll-interval`) between keypress
and status-bar update, which would feel wrong for an interactive
action. `show-options -pqv` is fast enough (~1 ms per pane) to
query every render.

Do NOT add a `50-marks.sh` provider. Mark state belongs inline.
